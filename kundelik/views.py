...
def profile_edit(request):
    ...
    else: user_form = UserEditForm(instance=user); profile_form = UserProfileEditForm(instance=user_profile, user=user)
...
def edit_daily_grade(request, pk):
    ...
    if not can_edit: raise PermissionDenied("Бұл бағаны өңдеуге рұқсатыңыз жоқ.")
    ...
    if request.method == 'POST':
        ...
        form.fields['student'].disabled = True; form.fields['subject'].disabled = True
        form.fields['date'].disabled = True
        if 'teacher' in form.fields: form.fields['teacher'].disabled = True
        if form.is_valid():
            try:
                edited_grade = form.save(commit=True)
                messages.success(request, f"{grade_object.student.get_full_name()} оқушысының {grade_object.subject.name} пәнінен ({grade_object.date.strftime('%d.%m.%Y')}) алған бағасы сәтті өзгертілді.")
                redirect_url = reverse('add_daily_grade')
                try:
                     grade_date_str = grade_object.date.strftime("%Y-%m-%d")
                     student_class_pk = grade_object.student.userprofile.school_class.pk
                     subject_pk = grade_object.subject.pk
                     redirect_url += f'?class_id={student_class_pk}&subject_id={subject_pk}&date={grade_date_str}'
                except Exception as e: logger.warning(f"Не удалось сформировать URL для редиректа в edit_daily_grade: {e}")
                return redirect(redirect_url)
                 ...
                 messages.error(request, _("Бағаны сақтау мүмкін болмады."))
            except Exception as e: logger.exception("[Edit Daily Grade] Unexpected error saving grade:"); messages.error(request, f"Бағаны сақтау кезінде қате: {e}")
        else:
        ...
        form.fields['date'].disabled = True
        if 'teacher' in form.fields: form.fields['teacher'].disabled = True
    context = {'form': form, 'grade': grade_object, 'current_view': 'edit_daily_grade', 'page_title': f"{grade_object.student.get_full_name()} бағасын өңдеу ({grade_object.date.strftime('%d.%m.%Y')})"}
    return render(request, 'daily_grade_edit.html', context)
...
def edit_exam_grade(request, pk):
    ...
    if request.method == 'POST':
        ...
        form.fields['student'].disabled = True; form.fields['subject'].disabled = True
        form.fields['date'].disabled = True; form.fields['term'].disabled = True
        form.fields['exam_type'].disabled = True
        if 'teacher' in form.fields: form.fields['teacher'].disabled = True
        if form.is_valid():
            try:
                edited_grade = form.save(commit=True)
                messages.success(request, f"{grade_object.student.get_full_name()} оқушысының {grade_object.subject.name} пәнінен ({grade_object.get_exam_type_display()}, {grade_object.term}-тоқсан) алған бағасы сәтті өзгертілді.")
                redirect_url = reverse('add_exam_grade')
                try:
                     student_class_pk = grade_object.student.userprofile.school_class.pk
                     subject_pk = grade_object.subject.pk
                     term = grade_object.term; exam_type = grade_object.exam_type
                     redirect_url += f'?class_id={student_class_pk}&subject_id={subject_pk}&term={term}&exam_type={exam_type}'
                except Exception as e: logger.warning(f"Не удалось сформировать URL для редиректа в edit_exam_grade: {e}")
                return redirect(redirect_url)
            except Exception as e: logger.exception("[Edit Exam Grade] Unexpected error saving grade:"); messages.error(request, f"Бағаны сақтау кезінде қате: {e}")
        else:
             messages.error(request, "Форма толтыруда қателер бар.")
        ...
        form.fields['exam_type'].disabled = True
        if 'teacher' in form.fields: form.fields['teacher'].disabled = True
    context = {'form': form, 'grade': grade_object, 'current_view': 'edit_exam_grade', 'page_title': f"{grade_object.student.get_full_name()} {grade_object.get_exam_type_display()} бағасын өңдеу"}
    return render(request, 'exam_grade_edit.html', context)
